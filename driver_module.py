...
class MoveDriver():
    ...
    def login_account(self, driver, user, passwd):
        login_container = driver.find_element(By.CLASS_NAME, 'login-droite-contenu')
    ...
    def reservation_court(self, driver):
        menu_container = driver.find_element(By.ID, 'menugauche')
        ...
        for menu_button in menu_buttons:
            ...
            if 'Badminton / Pickleball' in text_in:
                menu_button.click()
        time.sleep(4)
        reservation_table = driver.find_element(By.CLASS_NAME, 'tableres')
        ...
        for a in a_reservations_table:
            ...
            if 'AJOUTER UNE RÉSERVATION' in text_in:
                a.click()
        time.sleep(4)
        reservation_sport_select = driver.find_element(By.ID, 'popuptest')
    ...
    def select_parteinaire(driver):
        time.sleep(4)
        dropdown = driver.find_element(By.CLASS_NAME, 'popup-content')
    ...
    def reservation_hours(self, driver, hour_text):
        driver.get('https://caps-portail.uqtr.ca/capnet/login.coba')
        menu_container = driver.find_element(By.ID, 'menugauche')
        menu_buttons = menu_container.find_elements(By.TAG_NAME, 'li')
        for menu_button in menu_buttons:
            text_in = menu_button.text
